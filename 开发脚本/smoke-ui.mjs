...
for (const s of scenarios) {
  try {
    ...
    /* 契合度四条：条子真的会按分数填满（满分 100，50 分就填一半），数字不超过 100 */
    const barRows = [...resultHtml.matchAll(/<div class="row"><span>([^<]+)<\/span><span class="track"><span class="fill" style="width:(\d+)%"><\/span><\/span><span>(\d+)<\/span><\/div>/g)];
    assert(barRows.length === 4, s.key + ' 契合度条渲染出 4 条（主菜/预算/搭配/店铺）');
    assert(barRows.every(r => +r[3] <= 100), s.key + ' 分数封顶在 100：' + barRows.map(r => r[3]).join('/'));
    assert(barRows.every(r => +r[2] === Math.max(4, +r[3])), s.key + ' 条长和分数成比例（满分 100，50 分就填一半）');
